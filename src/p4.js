...
/* ---- ALWAYS FULLSCREEN — ANY TOUCH TO THE GAME WINDOW ----
   ...
   on desktop via a `body.touch` guard) and, on touch devices, also locks landscape. Fullscreen needs
   a user gesture, so every start/resume also arms a one-shot that the first interaction completes.
   Beyond that, a global listener means ANY tap/click/touch anywhere on the game window — on a game
   button, on the touch UI, or on empty ground — snaps to fullscreen whenever it isn't already. */
function enterFullscreen() {
  try {
    ...
    if ((root.requestFullscreen || root.webkitRequestFullscreen) && !cur) {
      ...
      if (pr && pr.then) pr.catch(() => { });   // no gesture yet — listeners below retry on the next interaction
    }
  } catch (e) { }
...
/* ANY touch/press on the whole window enters fullscreen (skip only while typing a wolf's name).
   Capture phase so it fires even when a game/touch button calls stopPropagation() on its own pointerdown. */
addEventListener('pointerdown', () => { if (!document.fullscreenElement && !document.webkitFullscreenElement) enterFullscreen(); }, { passive: true, capture: true });
addEventListener('keydown', e => {
  const t = e.target;
  if (t && (t.tagName === 'INPUT' || t.tagName === 'TEXTAREA' || t.isContentEditable)) return;   // the name field — don't fullscreen mid-keystroke
