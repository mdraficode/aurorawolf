  ...
  update(dt, input, camYaw, camPitch) {
    ...
    if (sprint) {
      /* Spring Steps (the Ancient Stag's / Eagle Legend's gift) — sprint stamina
         drains 25 % slower. SPEEDRUN FIX v6.7: the gift was granted and described
    ...
    if (moving) {
      /* Thunder Charge (the Thunder Bison's / Lion Legend's gift) — sprint 12 %
         faster. SPEEDRUN FIX v6.7: was a dead perk (name + description only); it is
...
const AnimalDetection = {
  threat(a) {          // 0 unaware · 0.25 uneasy · 0.5 alert · 1 critical
    ...
    if (wolf.crouch) range *= wolf.perks.shadowStep ? 0.22 : 0.45;   // low and quiet — Shadow Step (the Shadow Wolf's / Leopard Legend's gift) makes prowling nearly invisible. SPEEDRUN FIX v6.7: the perk was granted but never read anywhere.
