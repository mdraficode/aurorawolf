...
(function () {
  ...
  setInterval(() => {
    try {
      ...
      // story diffs → events
      if (story.biomes.size === 0 || !story.biomes.has(curBiomeKey)) { story.biomes.add(curBiomeKey); log('biome-enter', { biome: curBiomeKey, n: story.biomes.size }); }
  ...
  badge.textContent = '● LIVE — 🤖 AUTOPILOT v7 TRUE HUNTER';
    ...
    try {
      ...
      if (!bot.goalOverride) for (const cq of QUESTS.active) {
        if (cq.kind === 'hunt') {
          ...
          else if (hit.a && hit.d < bestD) { bestD = hit.d; q = cq; goal = { x: hit.a.pos.x, z: hit.a.pos.z }; targetAnimal = hit.a; mode = 'hunt'; bot.huntStick = { a: hit.a, at: SIMNOW() }; }
          else if (!hit.a) {
            const t2 = bot.travelToBiome(cq.biome);
            if (t2 && (bestD === 1e9 || t2.d + 120 < bestD)) { bestD = t2.d + 120; q = cq; goal = t2; mode = 'travel'; }
          }
      ...
      if (!bot.goalOverride && (mode === 'wander' || (mode === 'travel' && !q && !bot.bossPilgrim && bestD > 420))) {
        const easy = nearestAnimal(a => (a.sp.hp || 1) <= 2 && (!a.asleep || true));
      ...
      for (const off of [0, 0.5, -0.5, 1.0, -1.0, 1.5, -1.5, 2.1, -2.1]) {
        let sc = probe(desired + off, look) + Math.abs(off) * 0.22;
      ...
      keys.ShiftLeft = !stalk && ((dg > 35 && wolf.stamina > 30 && !bot.drinking) || (closeSprint && wolf.stamina > 12)) && !wolf.exhausted;
      ...
      if (targetAnimal && mode !== 'rival') {
        ...
        if (d < 3.2 + (targetAnimal.sp.scale || 1) * 0.7) {
          ...
          if (SIMNOW() - bot.lastAtk > 650) {
            ...
            setTimeout(() => {
              if (targetAnimal.dead) log('kill', { sp: q ? (q.species || q.kind) : 'xp-hunt', msg: 'caught ' + (q && q.species ? q.species : (targetAnimal.sp.label || 'prey')) + (q ? ' → quest ' + q.have + '/' + q.need : ' · xp hunt') + ' · meat ' + inv.meat });
      ...
      bot.goalText = mode + ' → ' + dg.toFixed(0) + 'm' + (q ? ' (' + q.have + '/' + q.need + ')' : '') + (stalk ? ' 🐾stalking' : '');
