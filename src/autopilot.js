  ...
  setInterval(() => {
    try {
      ...
      if (wantFlee || fleeing) {
        ...
        if (!bot.fleeLog || SIMNOW() - bot.fleeLog > 15000) { bot.fleeLog = SIMNOW(); log('flee', { msg: 'badly hurt — disengaging', pack: pN || 0, d: Math.round(pred.d) }); }
        applyAim(); return;
      }
      ...
      if (hChase && pred.d < 34 && !bot.huntCombat && bossHit.d > 90 && frac > fleeAt) {
        RAFZ.bump('yield');
        const away = Math.atan2(wolf.pos.x - pred.a.pos.x, wolf.pos.z - pred.a.pos.z) + (bot.giveSide || 0);
        aim(away, 0.9);
        keys.KeyW = true; keys.ShiftLeft = wolf.stamina > 25 && !wolf.exhausted && pred.d < 26;
        wolf.crouch = false;
        bot.goalText = '⚔️ breaking clean from a hunting ' + (pred.a.sp ? pred.a.sp.label : 'apex') + ' (' + pred.d.toFixed(0) + 'm)';
        if (!bot.giveSide || Math.random() < 0.02) bot.giveSide = (Math.random() < 0.5 ? 1 : -1) * (0.3 + Math.random() * 0.5);
        applyAim(); return;
      }
