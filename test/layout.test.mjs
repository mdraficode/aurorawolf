...
R.layout = await mp.evaluate(() => {
  const box = id => { const e = document.getElementById(id); const r = e.getBoundingClientRect(); return { l: r.left, t: r.top, r: r.right, b: r.bottom }; };
  ...
  return {
    ...
    topbarVsInv: ov(top, inv), pauseVsStats: ov(pause, stats), promptInFreeSpace: !ov(prompt, joy) && !ov(prompt, btns),
