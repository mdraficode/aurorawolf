...
# usage: bash test/rafzzer_train.sh <startGen> <endGen>
set -u
cd /home/user
G=${1:?start}; END=${2:?end}
while [ "$G" -le "$END" ]; do
  STATUS=FAIL
  for A in 1 2 3; do
    node test/rafzzer_gens.mjs spawn "$G" "$A" >/dev/null 2>&1
    if node test/rafzzer_gens.mjs gate "$G" >/tmp/gate$G.log 2>&1; then STATUS=PASS; break; fi
    python3 - <<PY
import json,pathlib
...
PY
  ...
  node test/rafzzer_gens.mjs run "$G" 480 > /tmp/run$G.log 2>&1
  tail -2 /tmp/run$G.log
  VERDICT=$(python3 test/rafzzer_verdict.py "$G" 2>&1)
  echo "$VERDICT"
