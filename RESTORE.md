...
Everything in this workspace now lives on GitHub — repo `mdraficode/aurorawolf-v2`, branch `main`
(the Pages site serves index.html from the same repo). Only `.ghtoken` and this file are local-only.

    TOK=$(cat ~/.ghtoken)
    rm -rf /tmp/work && git clone https://x-access-token:$TOK@github.com/mdraficode/aurorawolf-v2.git /tmp/work
