...
**Touch devices** get an on-screen controller: left **virtual joystick** (partial deflection = quieter stalking), right-side **ATTACK / JUMP / GATHER / SPRINT / HOWL / SENSE / PROWL** buttons, top-right **minimap** beside the **🎒 satchel button** (or **I**) — tap it to open your inventory panel with live counts — drag to look, pinch to zoom, ⏸ pause. The controls stay out of your way: the joystick is invisible until your thumb lands on it — its invisible catch field now covers **most of the bottom-left quadrant** of the screen, and the ring springs up wherever your thumb lands, so you can start moving from almost anywhere down-left — a slow drag there steers the wolf, while a fast, steep slide that exits the field is understood as a camera pan (swipe up from there to look at the sky) — and **action buttons lock on press**: hold SPRINT (or any button) and slide your finger off it; the button stays held *and* the finger becomes a camera drag, so you can steer while you sprint or bite. A second finger pans too; lifting releases the button — and the action buttons rest faint — touching any of them wakes the whole cluster for a moment. The **HP/stamina bars sit in the upper-left corner** (stacked under the quest book, above the inventory), leaving the whole lower-left free for the stick.
...
shots/                  ← current-build screenshots (cave crystals · night aurora · waterfall · forest · v6.9 sky peak · fjord · cave bats · jumping fish)
...
- **☄️ Gifts of sky and old magic** — the mystic night events arm you for the Legend fight: track down the **fallen star** (the ☄️ meteor site) for **Deep Bite — +1 permanent bite damage**, and let the **white stag** meet your eyes up close (12 m) for **Wild-Hardened — +5 permanent max HP** (both survive death, never stacking twice). And a **bonded pack** shields you from *every* boss blow now — bites, charges, dives, eruptions: a packmate can throw itself under them all
...
- **🕳️ Caves & their bats** — crystal-lined caverns hide under the hills (enter a dark mouth, explore, den there); **v6.9** doubled the cave mouths across the mountain ranges — and the deep ones shelter **bat colonies**: roosting upside-down, wings idly folding. Walk under a roost and a bat makes ONE swooping pass at you (2 dmg) before wheeling home — they defend, they never chase
...
- **Infinite procedural terrain** — simplex-noise continents, ridged mountains, lakes, rivers of hills; chunks stream in/out as you roam, so exploration never ends. **v6.9:** the high ranges now crest past **110 m — sky-touching, snow-capped peaks** — and **fjords**: long, steep-walled sea arms carve straight through the mountains, so the coast reaches deep into the high country
...
- **Hunting with real combat** — prey fights back with speed: rabbits and hares are easy catches, foxes and goats need a chase, elk/deer/reindeer take several bites but drop the richest bounty (meat, pelts, bones). Enemies fight fair: bears and rival wolves chase you down, plant their feet and bite — from the front, the flank **and from behind**
...
- **Rivers & lakes** — winding river channels carve through lowlands (contour-noise bands), ponds and lakes in basins. **Swimming 2.0**: stamina drains while swimming; hit zero and your health starts slipping; paddle toward a bank to clamber out; **E** at the water's edge to drink and restore stamina. **v6.9:** lakes and rivers **jump with fish** — schools cruise the deep water and leap every few seconds; you can catch one with your strike **only while you're in the water or the shallows** (+1 meat, +6 XP)
- **🗺️ Landmarks** — rare procedural points of interest: Elder Tree, Stone Circle, Cave Mouth, Ruined Shrine, Fallen Log Bridge — each with its own model, biome affinity, and special resources nearby. **v6.9:** **Cliff Hangers** — stacked waystone cairns with a gold flag on the grand high points (40 m+, real drop), the best views in the world — and **Cliff Falls**, water sheeting off the peaks into pools, rivers and the sea. Both are marked 🏔️/🌊 on the minimap AND the big map, found or not, and pay **+25 XP** when you reach them (first find also heals)
...
- **🎮 Full-side joystick** — touch anywhere on the left of the screen and the stick wakes under your thumb: swipe as far as you like, it never lets go until you lift. Only a soft glowing tip shows while steering — the ring stays out of your view
- **🕊️ A living sky** — small birds flap between the trees: they perch on branches, hop the meadows pecking seeds, and burst into flight when you come too close. Each beast of the wild now speaks its own voice when you draw near
- **🎸 The signature tune** — a soft plucked-string lullaby in A minor, composed for these wilds (AABA' over an Am–F drone), plays wherever you roam — the game's own melody, night-detuned and aurora-sparkled
- **🎵 A soothing score** — gentle pads and slow melody carry the day (the constant wind hiss is gone); rain, thunder, water and animal voices remain, all synthesized live
- **📜 Quest guidance** — an accepted deed draws a **dotted gold line on your map** to where it can be completed, plus a **faint guiding arrow on the ground** beneath your wolf — you always know which way the deed calls
- **🤖 AI watch mode** — don't want to play? Press the 🤖 button and **the wolf plays itself**: a real-player AI stalks prey from behind, ambush-crits, picks sensible deeds, flees bears, rests, drinks, and chases the legend arc while you watch. A live feed narrates kills, levels and completed deeds; touch controls step aside; press ⏸ (or 🤖 again) and the wolf is instantly yours
...
- **📜 Feasible quests only** — hunts name quarry that actually lives where you're sent (and prefers herds nearby), explorations only name landmarks that exist in the world, collect deeds only ask for surface-gatherable goods — no more impossible deeds
...
```
Headless smoke/gate tests live in `test/` (Playwright): `npm test` runs the 27-suite gate; single suites e.g. `node test/smoke.mjs`, `node test/collision.test.mjs`, `node test/effects_visual.test.mjs`, `node test/v69_features.test.mjs` (v6.9 peaks/fjords/bats/vistas/fish laws) (env: `npm install && bash test/browserlab/boot.sh`)
